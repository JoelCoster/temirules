...
class Memory {
     ...
     * Returns the updated conversation history list.
     */
    fun addToConversationHistory(role: String, content: String): List<Pair<String, String>> {
        // Get existing conversation history or create new one
        val existingHistory = getStateParam("conversationHistory") as? MutableList<Pair<String, String>> 
            ?: mutableListOf()
        
        // Add new message
        existingHistory.add(Pair(role, content))
        
        // Keep only the last 10 messages
        while (existingHistory.size > 10) {
            existingHistory.removeAt(0)
        }
        
        // Update the state parameter
        setStateParam("conversationHistory", existingHistory)
    ...
    fun clearConversationHistory() {
        setStateParam("conversationHistory", mutableListOf<Pair<String, String>>())
    }
}
